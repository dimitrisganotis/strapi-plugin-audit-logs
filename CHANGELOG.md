...
- **Additional Admin Translations**: Added French (`fr`), Greek (`el`), Dutch (`nl`), Czech (`cs`), Spanish (`es`), Romanian (`ro`), Italian (`it`), Portuguese (`pt`), Polish (`pl`), Swedish (`sv`), and Hungarian (`hu`) locale files for the admin UI
